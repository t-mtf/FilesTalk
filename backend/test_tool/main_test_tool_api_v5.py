...
def configure_azure_settings(
    ...
    if azure_key is None:
        logger.error("Environment variable 'AZURE_OPENAI_API_KEY' is not set.")
...
def main_compare(
    ...
    try:
        ...
        df1 = pd.read_excel(
            reference_file_path, sheet_name="Results", dtype=str, na_values=[""]
        )
        df2 = pd.read_excel(
            input_file_path, sheet_name="Results", dtype=str, na_values=[""]
        )
        logger.warning(
            "df1_column_names: %s, df2_column_names: %s", df1.columns, df2.columns
        )
        df_merged = pd.merge(
            df1,
            df2,
...
def calculate_metrics(
    ...
    try:
        logger = logging.getLogger("logger")
        df = pd.read_excel(
            input_file_path, sheet_name="Sheet1", dtype=str, na_values=[""]
        )
        # to be removed
        input_file_path = (
            Path(".")
            / "data"
            / "output-276bdb54-43f6-4ab8-a63a-d0efe349fe5f-2025-06-30-13-02-24.xlsx"
        )
        df0 = pd.read_excel(
            input_file_path, sheet_name="Results", dtype=str, na_values=[""]
        )
...
def compare_two_extractions(
    ...
    output_file_path_synthesis1 = (
        Path(".") / "data" / "output-v5_modified_api-2025-06-30-14-21-09.xlsx"
    )
    output_file_path_synthesis2 = (
        Path(".")
        / "data"
        / "output-276bdb54-43f6-4ab8-a63a-d0efe349fe5f-2025-06-30-14-23-14.xlsx"
    )
    columns_to_compare = [
        "inflation_found",
        "inflation_type",
    ...
    logger.warning(
        "columns_to_keep: %s, columns_to_compare: %s",
        columns_to_keep,
        columns_to_compare,
    )
    main_compare(
...
def main():
    ...
    try:
        ...
        if compare_two_results:
            ...
            input_file_name1 = "v5_modified_api"
            file_suffix1 = "v5"
            input_file_name2 = "276bdb54-43f6-4ab8-a63a-d0efe349fe5f"
            file_suffix2 = "api-276"
            ...
            input_file_name = "v5_modified_api"  # extension .xlsx is not needed. (e.g. 276bdb54-43f6-4ab8-a63a-d0efe349fe5f)
            input_suffix = "v5"
            ...
            columns_to_compare = [
                "inflation found",
                "inflation type",
                "inflation effective date",
                "inflation notice period",
                "periodicity",
            ]
            columns_to_keep = ["contract_number", "customer_name"]
            ...
            #     Path(".") / "data" / "COMPARISON_v5_reference_results.xlsx"
            # )
